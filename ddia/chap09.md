...
### 三阶段提交
  两阶段提交被称为阻塞原子提交协议，因为2PC可能卡住并等待协调者恢复的情况。。已经提出一种被称为三阶段提交（3PC）的算法，但3PC假定网络延迟有界，节点相应有限；在网络延迟可能无限的系统中，它不能保证原子性。

  非阻塞原子提交需要一个完美的故障检测器（perfect failure detector）。
